    ...
    >
      ...
      <div className=" flex gap-5 flex-col-reverse items-end md:flex-row">
        <div
          className={` ${
            click
              ...
              : "w-0 px-0 opacity-0"
          } overflow-hidden duration-200 sm:duration-1000 bg-white rounded-lg sm:h-14 flex flex-col sm:flex-row p-5 sm:items-center sm:justify-between gap-4`}
        >
          {navbar.links.map(({ url, text }) => (
            <a className=" text-xs hover:text-teal-400 duration-500" href={url}>
              {text}
            </a>
          ))}
        </div>
        <div
          ...
          className=" w-14 h-14 bg-teal-200 rounded-xl flex items-center justify-center"
        >
          <i
            class={`fa-solid ${
              click ? "fa-xmark" : "fa-bars-staggered"
            } text-2xl`}
          ></i>
        </div>
      </div>
